  ...
  useEffect(() => {
    const check = async () => {
      try {
        ...
        // 🔑 Ensure uid is stored for ChatInput
        const userId = payload.id || payload._id || payload.sub;
        if (userId) {
          try { localStorage.setItem('uid', userId); } catch {}
        }
      } catch {
        try { localStorage.removeItem('token'); } catch {}
        setToken(null);
...
const userId = data.id || data._id || data.sub;
if (userId) {
  localStorage.setItem('uid', userId);
  console.log('[Auth] Stored uid from /users/me:', userId);
}
...
const login = async (newToken) => {
  ...
  const payload = decodeJwtPayload(newToken || '');
  console.log('[Auth] Decoded payload:', payload); // 🔎 Debug log

  setUser(payload);

  // Store whichever identifier exists
  const userId = payload.id || payload._id || payload.sub;
  if (userId) {
    localStorage.setItem('uid', userId);
    console.log('[Auth] Stored uid:', userId);
  } else {
    console.warn('[Auth] No userId found in payload');
  }
};
