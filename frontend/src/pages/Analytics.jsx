  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        <h1 className="text-2xl sm:text-3xl font-semibold text-cyan-400">Analytics</h1>

        <button
          className="rounded-lg bg-slate-700/60 border border-slate-600/50 text-slate-300 font-medium px-3 sm:px-4 py-2 sm:py-2.5 hover:bg-slate-700 transition-colors text-sm sm:text-base flex items-center gap-1.5 sm:gap-2"
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5 mb-4 sm:mb-6">
            <motion.div
              ...
              className="lg:col-span-2 bg-slate-900/60 rounded-lg p-4 sm:p-5 h-72 sm:h-96 lg:h-[26rem] border border-slate-800/50"
            >
              <h3 className="font-semibold text-slate-200 mb-2 sm:mb-3 text-sm sm:text-base">Inventory trend (last 15 days)</h3>
            ...
            <div className="space-y-4 sm:space-y-5">
              <motion.div
                initial={{ opacity: 0, y: 8 }}
                animate={{ opacity: 1, y: 0 }}
                className="bg-slate-900/60 rounded-lg p-4 sm:p-5 h-52 sm:h-56 border border-slate-800/50 flex flex-col items-center justify-center"
              >
                <h3 className="font-semibold text-slate-200 mb-2 text-sm sm:text-base">Status overview</h3>
              ...
              <motion.div
                initial={{ opacity: 0, y: 8 }}
                animate={{ opacity: 1, y: 0 }}
                className="bg-slate-900/60 rounded-lg p-3 sm:p-4 h-56 sm:h-64 border border-slate-800/50"
              >
                <h3 className="font-semibold text-slate-200 mb-2 text-sm sm:text-base">Items by Category</h3>
                <div className="h-[calc(100%-36px)]">
                  <Bar
                    data={barData}
                    options={{
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 mb-4 sm:mb-6">
            <motion.div className="bg-slate-900/60 rounded-lg p-4 sm:p-5 h-72 sm:h-[20rem] border border-slate-800/50" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
              <h3 className="font-semibold text-slate-200 mb-2 sm:mb-3 text-sm sm:text-base">High value (Top 10)</h3>
              <div className="h-[calc(100%-36px)]">
            ...
            <motion.div className="bg-slate-900/60 rounded-lg p-4 sm:p-5 h-72 sm:h-[20rem] border border-slate-800/50" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
              <h3 className="font-semibold text-slate-200 mb-2 sm:mb-3 text-sm sm:text-base">Low value (Top 10)</h3>
              <div className="h-[calc(100%-36px)]">
            ...
            <motion.div className="bg-slate-900/60 rounded-lg p-4 sm:p-5 h-72 sm:h-[20rem] border border-slate-800/50 sm:col-span-2 lg:col-span-1" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
              <h3 className="font-semibold text-slate-200 mb-2 sm:mb-3 text-sm sm:text-base">Upcoming expirations (by date)</h3>
              <div className="h-[calc(100%-36px)]">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mt-2">
            {[
              ...
              <motion.div key={s.k} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className={`rounded-lg p-3 sm:p-5 border ${s.color} hover:scale-105 transition-transform duration-200`}>
                ...
                <p className="text-2xl sm:text-3xl font-bold">{(data.statusCounts && data.statusCounts[s.k]) ?? 0}</p>
              </motion.div>
            ))}
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5 mt-4 sm:mt-5">
            <div className="bg-slate-900/60 rounded-lg p-4 sm:p-5 border border-slate-800/50">
              <div className="flex items-center gap-2 mb-3 sm:mb-4">
                <svg className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
                </svg>
                <h3 className="font-semibold text-sm sm:text-base text-slate-200">High value (Top 10)</h3>
              </div>
              <ul className="space-y-2 sm:space-y-3 text-sm">
                {data.topHighValue.map((i, idx) => (
                  <li key={i._id ?? idx} className="flex items-center justify-between p-2 rounded bg-slate-800/40 hover:bg-slate-800/60 transition-colors">
                    ...
                    <span className="font-semibold text-emerald-400 ml-2 flex-shrink-0">₹{Number(i.estimatedCost || 0).toFixed(2)}</span>
                  </li>
                ))}
            ...
            <div className="bg-slate-900/60 rounded-lg p-4 sm:p-5 border border-slate-800/50">
              <div className="flex items-center gap-2 mb-3 sm:mb-4">
                ...
                <h3 className="font-semibold text-sm sm:text-base text-slate-200">Low value (Top 10)</h3>
              </div>
              <ul className="space-y-2 sm:space-y-3 text-sm">
                {data.topLowValue.map((i, idx) => (
                  <li key={i._id ?? idx} className="flex items-center justify-between p-2 rounded bg-slate-800/40 hover:bg-slate-800/60 transition-colors">
                    <span className="truncate text-slate-300">{i.name} <span className="text-slate-500">({i.category})</span></span>
                    <span className="font-semibold text-blue-400 ml-2 flex-shrink-0">₹{Number(i.estimatedCost || 0).toFixed(2)}</span>
                  </li>
                ))}
            ...
            <div className="bg-slate-900/60 rounded-lg p-4 sm:p-5 border border-slate-800/50">
              <div className="flex items-center gap-2 mb-3 sm:mb-4">
                ...
                <h3 className="font-semibold text-sm sm:text-base text-slate-200">Upcoming expirations</h3>
              </div>
              <ul className="space-y-2 sm:space-y-3 text-sm">
                {data.upcomingExpirations.map((i, idx) => (
                  <li key={i._id ?? idx} className="flex items-center justify-between p-2 rounded bg-slate-800/40 hover:bg-slate-800/60 transition-colors">
                    <span className="truncate text-slate-300">{i.name} <span className="text-slate-500">({i.category})</span></span>
                    <span className="text-amber-400 font-medium ml-2 flex-shrink-0">{i.expiryDate ? new Date(i.expiryDate).toLocaleDateString() : '—'}</span>
                  </li>
                ))}
