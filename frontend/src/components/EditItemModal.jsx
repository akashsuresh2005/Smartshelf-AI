...
//   )
// }
// src/components/EditItemModal.jsx - FINAL VERSION
...
export default function EditItemModal({ item, onSaved, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-3 sm:p-4 z-50">
      ...
      <form onSubmit={save}
        className="w-full max-w-2xl bg-slate-900 rounded-xl p-4 sm:p-6 space-y-4 max-h-[90vh] overflow-y-auto">

        <div className="flex justify-between items-center">
          <h3 className="text-base sm:text-lg font-semibold text-cyan-400">Edit Item</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          ...
          <input type="date" className="input w-full" value={form.expiryDate} onChange={(e)=>set('expiryDate',e.target.value)} />
        </div>

        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
          <input type="number" className="input w-full" value={form.quantity} onChange={(e)=>set('quantity',e.target.value)} />
          <select className="input w-full sm:w-[120px]" value={form.unit} onChange={(e)=>set('unit',e.target.value)}>
