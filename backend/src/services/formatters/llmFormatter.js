...
/**
 * 🔥 STRICT PROMPT (forces clean structure)
 */
const wrapPrompt = (userMessage) => {
  return `
You are SmartShelf AI — a helpful assistant.
...
/**
 * 🔥 FORMAT CLEANER (guarantees output structure)
 */
function formatResponse(text = "") {
  ...
  // Fix numbering issues like "1. Summary"
  cleaned = cleaned.replace(/^1\.\s*/i, "Summary:\n");
  cleaned = cleaned.replace(/^2\.\s*/im, "\nDetails:\n");
  cleaned = cleaned.replace(/^3\.\s*/im, "\nSuggested Actions:\n");

  // Ensure proper spacing
  cleaned = cleaned.replace(/Summary:/i, "Summary:\n");
  cleaned = cleaned.replace(/Details:/i, "\nDetails:\n");
  cleaned = cleaned.replace(/Suggested Actions:/i, "\nSuggested Actions:\n");
  ...
  try {
    const response = await axios.post(
      "https://api.groq.com/openai/v1/chat/completions",
      {
        ...
        messages: [
          { role: "system", content: "You are SmartShelf AI." },
          { role: "user", content: wrapPrompt(userMessage) }
        ],
        temperature: 0.6
      },
      {
