...
// import { buildWhatsAppMessage } from '../utils/whatsappFormatter.js';
...
// export async function sendWhatsAppAlert(req, res, next) {
//   try {
...
//     await sendWhatsApp(phone, message, item.imageUrl);

//     res.json({ ok: true, message: "WhatsApp alert sent" });
//   } catch (err) {
//     next(err);
//   }
// }
...
export async function list(req, res, next) {
  try {
    ...
    const perPage = toInt(limit, 20);
    const skip = (pageNum - 1) * perPage;

    const [rows, total] = await Promise.all([
      Notification.find(find)
  ...
  try {
    ...
    if (!user.phone || !user.whatsappEnabled) {
      return res.status(400).json({
        error: 'WhatsApp disabled or phone missing'
      });
    }

    const message = buildWhatsAppMessage(item);

    await sendWhatsApp(user.phone, message, item.imageUrl);
